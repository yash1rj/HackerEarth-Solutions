...
The Monk is given N numbers and has to select K of these numbers. For each number that Monk chooses, 
he will get as many cheesecakes as the number of 1's in the Binary representation of the number i.e. the number of bits that are set.
...
for i in range(t):
    ...
    a=[str(bin(int(x))).count("1") for x in input().split(" ")]
    a=sorted(a)
    if k!=0:
        print(sum(a[-k:]))
    else:
